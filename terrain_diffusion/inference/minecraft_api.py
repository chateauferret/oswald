...
def main(model_path, hdf5_file, caching_strategy, cache_size, seed, device, batch_size, log_mode, torch_compile, dtype, host, port, extra_kwargs):
    ...
    if hdf5_file is not None:
        ...
        if hdf5_file != 'TEMP' and not os.path.isabs(hdf5_file):
    ...
    if dtype == 'fp32':
        dtype = None
    _PIPELINE_CONFIG = {
        'model_path': get_checkpoint_path(model_path) if os.path.isdir(model_path) or model_path.startswith('checkpoints/') else model_path,
        'hdf5_file': hdf5_file,
        'caching_strategy': caching_strategy,
