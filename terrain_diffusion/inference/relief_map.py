...
def get_relief_map(
    ...
    *,
    ...
    Returns:
        shaded_rgb: (H, W, 3) float32 RGB array.
    """
    ...
    if rgb is None:
        ...
        # Use provided vmin/vmax or defaults. We avoid auto-scaling here to ensure
        ...
        # For the 'topo' colormap, 0.5 is the shoreline (0m elevation).
        # We map land elevation [0, vmax] to [0.5, 1.0] in the colormap.
        # We use a power transform (0.7) to give more detail to lower elevations.
        norm_cmap = 0.5 + np.clip(norm ** 0.7, 0.0, 1.0) * 0.5
        rgb = topo(norm_cmap)[..., :3].astype(np.float32)
