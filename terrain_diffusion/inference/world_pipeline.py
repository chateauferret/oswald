...
class WorldPipeline(ConfigMixin):
    ...
    def __init__(
        ...
        latents_batch_size: int | list[int] = [1, 4],
    ...
    def to(self, device):
        ...
        _move(self.coarse_model)
        _move(self.base_model)
        _move(self.decoder_model)
    ...
    def empty_cache(self, clear_cuda: bool = True):
        """Clear the cache for all tensors in the pipeline and optionally free CUDA memory."""
        for tensor in (self.coarse, self.latents, self.residual):
            if tensor is not None and hasattr(tensor, 'clear_cache'):
                tensor.clear_cache()
    ...
    def _coarse_inference(self, ctxs, scheduler, weight_window, t_cond, cond_inputs, pool_size=1):
        """Run inference for coarse tiles."""
        ...
        actual_batch_size = len(ctxs)
        model = self.coarse_model
        device = next(model.parameters()).device
        model_dtype = self._dtype or next(model.parameters()).dtype
        
        if self.log_mode == 'verbose':
            print(f"Coarse f batch size {actual_batch_size} at {ctxs}")
        ...
        for ctx in ctxs:
            ...
            sm = self._conditioning_model_input(i1, i2, j1, j2)
            sm = (sm - MODEL_MEANS[[0, 2, 3, 4, 5], None, None]) / MODEL_STDS[[0, 2, 3, 4, 5], None, None]
            synthetic_maps.append(sm[None])
            
            cn = torch.as_tensor(gaussian_noise_patch(
                self.seed, i1, j1, TILE_SIZE, TILE_SIZE, 
                channels=5, tile_h=TILE_SIZE, tile_w=TILE_SIZE
            ))[None]
            cond_noises.append(cn)
            
            sn = torch.as_tensor(gaussian_noise_patch(
                self.seed + 1, i1, j1, TILE_SIZE, TILE_SIZE,
                channels=6, tile_h=TILE_SIZE, tile_w=TILE_SIZE
            ))[None]
            sample_noises.append(sn)
            
        synthetic_map_batch = torch.cat(synthetic_maps, dim=0).to(device, dtype=model_dtype)
        cond_noise_batch = torch.cat(cond_noises, dim=0).to(device, dtype=model_dtype)
        sample_noise_batch = torch.cat(sample_noises, dim=0).to(device, dtype=model_dtype)
        
        t_cond_view = t_cond.view(1, -1, 1, 1).to(device, dtype=model_dtype)
        cond_img_batch = torch.cos(t_cond_view) * synthetic_map_batch + torch.sin(t_cond_view) * cond_noise_batch
        
        scheduler.set_timesteps(20)
        samples = sample_noise_batch * scheduler.sigmas[0].to(device, dtype=model_dtype)
        
        for t, sigma in zip(scheduler.timesteps, scheduler.sigmas):
            t = t.to(device, dtype=model_dtype)
            sigma = sigma.to(device, dtype=model_dtype)
            scaled_in = scheduler.precondition_inputs(samples, sigma)
            cnoise = scheduler.trigflow_precondition_noise(sigma.view(-1))

            x_in = torch.cat([scaled_in, cond_img_batch], dim=1).to(model_dtype)
            noise_labels = cnoise.expand(actual_batch_size).to(device, dtype=model_dtype)
            
            cond_inputs_gpu = [v.to(device, dtype=model_dtype) for v in cond_inputs]
            model_out = model(x_in, noise_labels=noise_labels, conditional_inputs=cond_inputs_gpu)
            samples = scheduler.step(model_out, t, samples).prev_sample
        
        samples = samples.cpu().float() / scheduler.config.sigma_data
        samples = (samples * MODEL_STDS.view(1, -1, 1, 1)) + MODEL_MEANS.view(1, -1, 1, 1)
        ...
        for i in range(actual_batch_size):
            s = samples[i]
            if pool_size > 1:
                s = self._pool_coarse_conditioning(s, pool_size)
            results.append(torch.cat([s * weight_window[None], weight_window[None]], dim=0))
        return results
    ...
    def _build_coarse_stage(self):
        ...
        def f(ctxs):
            return self._coarse_inference(ctxs, scheduler, weight_window, t_cond, cond_inputs, pool_size=coarse_pool)
    ...
    def _process_latent_conditioning(self, cond_img, histogram_raw, cond_means, cond_stds, noise_level, seed_offset: int = 0):
        ...
        return mp_concat([
            ...
            mask_crop.flatten(1), histogram_raw.to(cond_img.device, dtype=cond_img.dtype), 
            noise_level_norm.view(-1, 1)
        ], dim=1).float()
    ...
    def _latent_inference(self, ctxs, samples, cond_imgs, t, scheduler, weight_window, histogram_raw, cond_means, cond_stds, seed_offset=0):
        ...
        t_tensor = torch.as_tensor(t, dtype=model_dtype, device=device)
        
        for ctx, sample, cond_img in zip(ctxs, samples, cond_imgs):
            if sample is None:
                sample = torch.zeros((1, 5, TILE_SIZE, TILE_SIZE), device=device, dtype=model_dtype)
            else:
                sample = torch.as_tensor(sample, device=device, dtype=model_dtype)
                sample = sample[:-1] / sample[-1:] * scheduler.config.sigma_data
            
            cond_img = torch.as_tensor(cond_img, device=device, dtype=model_dtype)
            cond_img = cond_img[:-1] / cond_img[-1:]
            
            mask = torch.ones((1, 4, 4), device=device, dtype=model_dtype)
            cond_img = torch.cat([cond_img, mask], dim=0)[None]
            
            tile_seed_off = ctx[1] * 65536 + ctx[2]
            cond_inputs = self._process_latent_conditioning(
                cond_img, histogram_raw, cond_means, cond_stds, torch.tensor(NOISE_LEVEL, device=device, dtype=model_dtype), seed_offset=tile_seed_off
            ...
            z = noise.to(device, dtype=model_dtype) * scheduler.config.sigma_data
        ...
        noise_labels_batch = torch.full((padded_batch_size,), t_tensor.item(), device=device, dtype=model_dtype)
        ...
        pred_batch = -model(model_in_batch, noise_labels=noise_labels_batch, conditional_inputs=[cond_inputs_batch])
        ...
        for i in range(actual_batch_size):
            ...
            t_view = t_tensor.view(1, 1, 1, 1).to(device)
    ...
    def _decoder_inference(self, ctxs, latents_batch, scheduler, weight_window, t_list, tile_size, tile_stride):
        """Run inference for decoder tiles."""
        if not isinstance(ctxs, list):
            ctxs = [ctxs]
        ...
        actual_batch_size = len(ctxs)
        if self.log_mode == 'verbose':
            print(f"Residual f batch size {actual_batch_size} at {ctxs}")
        if MOCK:
            return [torch.ones((2, TILE_SIZE, TILE_SIZE)) for _ in ctxs]
        
        model = self.decoder_model
        device = next(model.parameters()).device
        model_dtype = self._dtype or next(model.parameters()).dtype
        
        samples = torch.zeros((actual_batch_size, 1, TILE_SIZE, TILE_SIZE), device=device, dtype=model_dtype)
        lc = self.latent_compression
        
        latents_tensors = []
        for l in latents_batch:
            l = torch.as_tensor(l, device=device, dtype=model_dtype)
            l = (l[:-1] / l[-1:])[:4].view(1, 4, TILE_SIZE//lc, TILE_SIZE//lc)
            latents_tensors.append(l)
            
        upsampled_latents = torch.nn.functional.interpolate(
            torch.cat(latents_tensors, dim=0), size=(TILE_SIZE, TILE_SIZE), mode='nearest'
        )
        ...
        for i, t in enumerate(t_list):
            ...
            for ctx in ctxs:
                ...
                noises.append(noise)
            z = torch.cat(noises, dim=0).to(device, dtype=model_dtype) * scheduler.config.sigma_data
            
            t_tensor = t.view(1, 1, 1, 1).to(device, dtype=model_dtype)
            x_t = torch.cos(t_tensor) * samples + torch.sin(t_tensor) * z
            model_in = torch.cat([x_t / scheduler.config.sigma_data, upsampled_latents], dim=1)
            
            noise_labels = torch.full((actual_batch_size,), t.item(), device=device, dtype=model_dtype)
            pred = -model(model_in, noise_labels=noise_labels, conditional_inputs=[])
            samples = torch.cos(t_tensor) * x_t - torch.sin(t_tensor) * scheduler.config.sigma_data * pred
            
        samples = samples.cpu().float() / scheduler.config.sigma_data
        
        results = []
        for i in range(actual_batch_size):
            results.append(torch.cat([samples[i] * weight_window[None], weight_window[None]], dim=0))
        return results
    ...
    def _build_decoder_stage(self):
        ...
        def f(ctxs, latents_batch):
            return self._decoder_inference(ctxs, latents_batch, scheduler, weight_window, t_list, TILE_SIZE, TILE_STRIDE)
