...
def write_tiff(path, arr, transform, crs="EPSG:4326", nodata=None, endian=None):
    options = {
        "driver": "GTiff",
        "height": arr.shape[0],
        "width": arr.shape[1],
...
def azgaar_to_tiff(input_json, output_dir, scale=100.0, ocean_max_depth=4000.0, ocean_power=1.5, endian=None):
    """Convert an Azgaar full JSON export to GeoTIFF rasters.

    Args:
        input_json: Path to the Azgaar JSON export.
        output_dir: Directory where the TIFFs will be written.
        scale: Size of each output pixel in km.
        ocean_max_depth: Maximum ocean depth in meters.
        ocean_power: Power curve exponent for ocean depth.
        endian: Force output endianness ("LITTLE" or "BIG").
    """
    ...
    write_tiff(output_dir / "heightmap.tif", arr, transform, endian=endian)
    ...
    write_tiff(output_dir / "temperature.tif", arr, transform, endian=endian)
    ...
    arr = fill_nodata(arr, -9999.0)
    write_tiff(output_dir / "temperature_std.tif", arr, transform, endian=endian)
    ...
    arr = fill_nodata(arr, -9999.0)
    write_tiff(output_dir / "precipitation.tif", arr, transform, endian=endian)
    ...
    arr = fill_nodata(arr, -9999.0)
    write_tiff(output_dir / "precipitation_cv.tif", arr, transform, endian=endian)
...
def main(input, output_dir, scale, ocean_max_depth, ocean_power, endian):
    """Convert an Azgaar full JSON export to GeoTIFF rasters."""
    azgaar_to_tiff(input, output_dir, scale, ocean_max_depth, ocean_power, endian)
