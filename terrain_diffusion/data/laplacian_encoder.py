...
try:
    ...
    # Fallback for old torchvision (< 0.9.0)
...
def resize_extrapolated(x, size, interpolation=InterpolationMode.BILINEAR, **kwargs):
    if not isinstance(size, (tuple, list)):
        if isinstance(x, torch.Tensor):
            return TF.resize(x, [size, size], interpolation=interpolation, **kwargs)
...
def laplacian_encode(x, downsample_size, sigma, interp_mode=InterpolationMode.BILINEAR, extrapolate=False):
...
def laplacian_decode(residual, lowres, interp_mode=InterpolationMode.BILINEAR, extrapolate=False, pre_padded=False):
...
def laplacian_denoise(residual, lowres, sigma, interp_mode=InterpolationMode.BILINEAR):
