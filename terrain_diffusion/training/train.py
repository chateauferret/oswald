...
def resolve_config_paths(config):
    """Recursively resolve all strings starting with 'data/' or 'checkpoints/'."""
    if isinstance(config, (dict, Config)):
        for k, v in config.items():
            if isinstance(v, (dict, list, Config)):
                resolve_config_paths(v)
            elif isinstance(v, str):
                if v.startswith('data/'):
                    config[k] = get_data_path(v)
                elif v.startswith('checkpoints/'):
                    config[k] = get_checkpoint_path(v)
        ...
        for i, v in enumerate(config):
            if isinstance(v, (dict, list, Config)):
                ...
                if v.startswith('data/'):
                    config[i] = get_data_path(v)
                elif v.startswith('checkpoints/'):
                    config[i] = get_checkpoint_path(v)
...
def main(ctx, config_path, ckpt_path, model_ckpt_path, debug_run, resume_id, override):
    ...
    # Resolve save_dir (redundant if resolve_config_paths works, but safe)
    if 'logging' in config and 'save_dir' in config['logging']:
        config['logging']['save_dir'] = get_checkpoint_path(config['logging']['save_dir'])
