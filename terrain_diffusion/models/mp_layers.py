...
def mp_sum(args, w=None):
    ...
    # Use broadcasting instead of stack to support mixed batch sizes (e.g., [N, D] and [1, D])
    res = 0
    for a, weight in zip(args, w):
        res = res + a * weight
    return res / torch.linalg.vector_norm(w)
...
def mp_concat(args, dim=1, w=None):
    ...
    # Find max batch size at dimension 0 to support broadcasting if some inputs have batch size 1
    if dim != 0:
        batch_sizes = [x.shape[0] for x in args]
        max_bs = max(batch_sizes)
        if max_bs > 1:
            args = [x.expand(max_bs, *x.shape[1:]) if x.shape[0] == 1 else x for x in args]
